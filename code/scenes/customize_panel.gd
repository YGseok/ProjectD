...
##
## move_to_front(): 이 패널을 부르는 화면(dungeon_map 등)이 _ready() 이후에 자신의
## 자식 노드를 더 추가하면(예: dungeon_map.gd의 _setup_reward_icons()가 방 선택 버튼
## 옆 보상 아이콘을 add_child()로 붙임) 그 노드들이 씬 트리에서 이 패널보다 뒤에 와서
## 더 나중에(= 위에) 그려진다 — 그러면 이 패널이 열려 있어도 나중에 추가된 형제 노드가
## 화면을 뚫고 보인다(INBOX.md 2026-09-14 "던전 맵 커스터마이징 팝업을 띄웠는데,
## 전투 방 입장 보상 아이콘이 팝업 창을 뚫고 나온다"). 열릴 때마다 자신을 부모의 맨
## 마지막 자식으로 옮겨(= 맨 위로 그려지게) 이후에 추가된 형제가 몇 개든 항상 이
## 패널이 최상단에 오도록 보장한다.
func open() -> void:
	move_to_front()
	visible = true
