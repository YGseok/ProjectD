...
## "explosive_stack"(폭발병)/"guard_stack"(방패병)은 여기서 다룰 정적 다이스 개조가
## 없어(면 값을 바꾸는 게 아니라 전투 중 턴마다 스택을 추적하는 방식) 아래 match에
## 걸리지 않고 그대로 "_: pass"로 빠진다 — 실제 적용은 combat_test.gd가
## CharacterProfiles.get_profile()을 직접 읽어서 처리한다(character_profiles.gd
## 클래스 주석 참고).
