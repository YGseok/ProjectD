...
- 작성자: AI 에이전트 (INBOX.md에 이번에 새로 반영할 미처리 항목이 없어 — 남은 두
  ...
  STATUS.md 다음 할 일 큐 1번의 "순수 텍스트형/스토리형 선택지 이벤트 방 종류 추가"를
  진행함. 던전 맵에 "스토리 이벤트" 네 번째 선택지 추가 — 자세한 내용은 "완료 기록" 참고)
...
1. **던전 맵 내용 채우기 (계속)** — "런에 끝이 있다"(`RunState.TOTAL_ROOMS=5`, 완료 시
   ...
   - 신규 "스토리 이벤트"의 시나리오 4종(낡은 보물상자/떠돌이 상인/휴식처/수상한
     도박꾼)과 골드 증감폭(±5~25, 확률 50~60%)은 감으로 잡은 잠정값 — 실제로
     플레이해보고 텍스트 톤이나 골드 임팩트가 적당한지 사람 피드백 필요. 시나리오
     가짓수가 4개뿐이라 반복 플레이 시 금방 질릴 수 있음(다음 이터레이션에서 늘릴 후보).
