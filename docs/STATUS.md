...
- 일시: 2026-09-02
- 작성자: AI 에이전트 (INBOX.md 최우선 반영: 프로젝트 폴더 재구성(docs/qa_out/code/
  resources/Util/scripts) + INBOX.md 자체를 남은 이슈/부분 처리됨/처리됨 3섹션으로
  재정리 — 자세한 내용은 "완료 기록" 참고)
...
- **던전 맵 방 선택지를 확률적으로 일부만 노출하도록 변경 완료** (이번 이터레이션,
...
1. **던전 맵 내용 채우기 (계속)** — "런에 끝이 있다"(`RunState.TOTAL_ROOMS=5`, 완료 시
   ...
   - ~~3개 방 선택지가 매번 전부 노출되는 대신, 방마다 일부만 무작위로 제시되거나
     순서가 섞이는 것이 "레벨 디자인"에 더 가까울 수 있음~~ → **완료됨** (이번
     이터레이션, 아래 "완료 기록" 참고). "전투 방"은 항상 노출(진행 보장), "상점"/
     "특수 이벤트"는 방마다 각각 60%/50% 확률로 독립 노출. 확률 자체가 적당한지,
...
11. **(INBOX.md 신규, 부분 완료) 레벨 디자인된 여러 스테이지** — "일단은 선형으로
    ...
    "전투 방" / "상점" / "특수 이벤트" 세 선택지를 보여주는 것과, 그 선택지가 방마다
    확률적으로 일부만 노출되는 것(이번 이터레이션, 큐 1 참고)까지는 완료. 다만 방
    종류 자체가 이 셋뿐이라 순수 텍스트형/스토리형 선택지 이벤트는 여전히 없음 —
    다음 단계로 그런 방 종류를 추가하는 것을 검토. 큐 1(던전 맵 내용 채우기)과
    병합해서 진행하는 것을 권장.
