...
- 일시: 2026-09-02
- 작성자: AI 에이전트 (다음 할 일 큐 2번 일부 반영: 다이스 개수가 늘어나도 벽 밖으로
  스폰되지 않도록 `_spawn_dice()`를 그리드 레이아웃으로 변경)
...
- **다이스 스폰 레이아웃을 그리드 방식으로 바꿔 다이스 개수가 늘어나도 벽 밖으로
  안 나가게 수정 완료** (이번 이터레이션, 큐 2 일부). 자세한 내용은 "완료 기록" 참고.
...
2. **전투 씬 다듬기 / 후속 검토**
   ...
   - 다이스 정지 감지는 구현했지만 임계값(`SETTLE_LIN_THRESHOLD`/`SETTLE_ANG_THRESHOLD`/
     ...
     주머니에 다이스 추가 등) 정지까지 걸리는 시간이 길어질 수 있어 재검증 필요.
