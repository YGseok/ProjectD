...
- 일시: 2026-09-02
- 작성자: AI 에이전트 (다음 할 일 큐 2번 계속: 다이스 개수가 늘어났을 때 정지 감지
  임계값 재검증 — 자세한 내용은 "완료 기록" 참고)
...
- **다이스 정지 감지 임계값을 다이스 개수를 늘려 재검증 완료** (이번 이터레이션, 큐 2
  ...
  개선했으나, 14개 이상 극단적인 경우는 여전히 항상 4초 안전장치로 빠짐 — 완전히
  해결된 문제는 아니고, 실전에서 한 주머니가 그 정도로 커질 일이 실제로 있는지는
  사람 피드백 필요. 자세한 내용은 "완료 기록" 참고.
...
2. **전투 씬 다듬기 / 후속 검토**
   ...
   - ~~다이스 정지 감지 임계값이 다이스 개수가 늘어나면 재검증 필요~~ → **재검증 완료**
     (이번 이터레이션, 아래 "완료 기록" 참고). 3~9개는 자연 정지 감지가 잘 동작, 14개
     이상은 매번 4초 안전장치로 빠짐(크래시/시각적 문제는 없으나 정지 감지 기능 자체가
     사실상 무력화됨). 임계값을 소폭 완화했지만 극단적인 다이스 개수 문제 자체는
     남아있음 — 실제 플레이에서 한 주머니가 14개 가까이 커질 수 있는지(현재 아이템
     시스템상 5방 런에서는 현실적으로 어려움) 사람 판단 필요.
...
- **다이스가 14개 이상으로 한 주머니에 몰리면 정지 감지가 사실상 항상 4초 안전장치로
  빠짐** (이번 이터레이션에서 재검증, 위 큐 2 참고). 서로 맞닿은 다이스들의 접촉
  잔진동 때문으로 추정 — 크래시나 화면 깨짐은 없고 게임은 정상 진행되지만, 정지
  감지가 의도한 "빨리 멈추면 빨리 넘어간다" 이점을 못 살리고 매번 최대 대기 시간을
  씀. 임계값을 한 차례 완화(`SETTLE_LIN_THRESHOLD`/`SETTLE_ANG_THRESHOLD`/
  `SETTLE_MIN_FRAMES`)했지만 8~9개 구간까지만 개선됐고 14개 구간은 여전함. 근본
  해결은 사람이 "한 주머니가 실제로 그 정도까지 커지는 게 의도된 플레이인지"부터
  판단해야 착수할 수 있어 보류.
