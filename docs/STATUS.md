...
- 일시: 2026-09-01
- 작성자: AI 에이전트 (큐 1번: 2D+3D 혼합 파이프라인 스파이크 완료)
...
1. **다이스 판정 로직 (순수 GDScript, 비주얼 없이 먼저)**
   - `res://systems/dice_bag.gd` 같은 스크립트로 "N개의 D면체를 굴려서 합계를 낸다"는
     로직을 구현. (주머니 구성: 면 개수 M, 개수 N을 파라미터로 받는 형태로)
   - 데미지 계산 함수: `damage = max(0, 공격_합계 - 방어_합계)` — DESIGN.md 공식 그대로.
   - 이 단계는 시각 요소 없이 값만 맞는지 확인 (print 또는 간단한 테스트 씬으로).

2. **다이스 하나의 물리 프리팹 (재질: 플라스틱)**
...
4. **위 3개가 안정되면 이후 큐 항목을 이 파일에 추가** (상점/이벤트, 방 이동, 아트 등).
...
- 물리 시뮬레이션 특성상 다이스가 멈추는 시점이 매번 달라짐 → `GAME_QA_FRAME`을
  ...
  더 늘려야 할 수 있음. "정지 감지" 방식은 아직 미구현 — 큐 3번(전투 씬) 진행 시 필요성
  재평가할 것.
- `res://scenes/dungeon.tscn`은 지금 스파이크/전투 테스트용 단일 씬으로 쓰고 있음.
  실제 던전 방 이동 등 본게임 구조가 생기면 이름/역할을 다시 정리해야 함 (큐 4번 이후).
