...
- [부분 처리됨 - 2026-09-17] 2026-09-17 스킬을 강화시킬 수 있는 이벤트를
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 D] 스킬 강화 이벤트)**
  ...
  → **1~3번 완료(2026-09-17, docs/STATUS.md 완료 기록 (128)/(129)), 4~6번 남음.**
  `code/systems/skill_pool.gd`에 `UPGRADE_SKILLS`(7종 강화판 데이터) +
  `SkillPool.available_upgrade_choices(n, character_id)`/`grant_upgrade(base_id)`
  두 함수를 추가했고((128)), 이어서 `code/scenes/event.gd`에
  `SKILL_UPGRADE_EVENT_CHANCE`(=0.2, 잠정값)로 특수 이벤트 방이 기존 스킬
  이벤트(30%)보다 먼저 "스킬 강화" 이벤트로 분기하도록 구현했다((129)) — 강화
  후보가 하나도 없으면 항상 기존 스킬/아이템 이벤트로 폴백, 카드 UI는 새로
  만들지 않고 기존 스킬 획득 카드 UI를 그대로 재사용. 아직 전투 배선(4번)/UI
  강조(5번)/선택 항목(6번)은 미착수라 "+" 스킬을 실제로 골라도 전투 효과는
  없다. 1~5번이 모두 끝나면 이 항목 전체를 "처리됨"으로 옮길 것(완료 기준,
  아래 원문 그대로 유지).
