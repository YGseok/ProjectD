...
- 2026-09-16 (기획자 결정) `docs/STATUS.md` "알려진 이슈"의 "특수 이벤트 B급(D8)
  아이템 사각지대" — 제시된 3가지 해결안 중 **(b)로 결정**: 안전 풀을 "C 이하",
  위험 성공 풀을 "B 이상"으로 등급 경계를 겹치게 넓힌다(등급 재배정(a)이나 D8
  제외(c)는 하지 않음). `code/systems/event_item_pool.gd`의
  `random_safe_item()`/`random_risky_item()`(또는 실제 함수명이 다르면 등가
  로직)이 지금 특정 grade 목록으로 하드코딩돼 있을 텐데, "안전 = grade in
  [C]" -> "grade in [C, D8이 속한 B 포함]"이 아니라 등급 순서 기준으로
  "안전 = 하위 절반 이하(C~B 경계)", "위험 성공 = 상위 절반 이상(B~A 경계)"처럼
  **B급이 두 풀 모두에 걸리도록** 경계를 겹치게 한다(정확한 grade 순서는
  `dice_item_pool.gd`/`event_item_pool.gd` 주석의 "S > A > B > C" 기준 그대로).
  이렇게 하면 안전 선택으로도(낮은 확률로) B급이 나올 수 있고, 위험 성공으로도
  B급이 나올 수 있어 사각지대가 없어짐 — 안전/위험의 "리스크 대비 리턴" 차이는
  "안전은 C가 훨씬 흔하고 B는 드묾", "위험 성공은 B/A/S가 나오고 상위 등급일수록
  흔함" 식의 가중치로 유지(정확한 확률/가중치는 구현하는 사람이 잠정값으로 정해도
  됨 — 사람 피드백으로 나중에 조정). 완료되면 `docs/STATUS.md`의 해당 "알려진
  이슈" 항목을 지우고 `dice_test.gd` 회귀 스위트에 "B급이 두 풀 모두에서 뽑힐 수
  있는지" 검증을 추가할 것.
...
최근 12개만 유지해 매 이터레이션 읽기 비용을 줄임. 이번 이터레이션(120)에서
"현재 게임에 적용된 캐릭터를 확인해볼 수 있도록..." 항목을 그리로 옮겼다.)*
