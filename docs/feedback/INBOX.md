...
- [부분 처리됨 - 2026-09-17] 2026-09-17 스킬을 강화시킬 수 있는 이벤트를
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 D] 스킬 강화 이벤트)**
  ...
  공용 2종(심호흡+/여분+, 130) + 고유 2종(광기 심화+/수호 심화+, 131) 배선
  완료, 나머지 고유 3종/UI 강조(5번)/선택 항목(6번) 남음.** `code/systems/
  skill_pool.gd`에 `UPGRADE_SKILLS`(7종 강화판 데이터) + `SkillPool.
  available_upgrade_choices(n, character_id)`/`grant_upgrade(base_id)` 두
  함수를 추가했고((128)), 이어서 `code/scenes/event.gd`에
  `SKILL_UPGRADE_EVENT_CHANCE`(=0.2, 잠정값)로 특수 이벤트 방이 기존 스킬
  이벤트(30%)보다 먼저 "스킬 강화" 이벤트로 분기하도록 구현했다((129)) —
  강화 후보가 하나도 없으면 항상 기존 스킬/아이템 이벤트로 폴백, 카드 UI는
  새로 만들지 않고 기존 스킬 획득 카드 UI를 그대로 재사용.
  **(130)에서 4번의 첫 조각으로 공용 스킬 2종의 "+" 효과를 `combat_test.gd`의
  ...
  reroll()` 순수 함수로 분리해 단위 테스트 가능하게 함. 남은 고유 스킬 3종
  (연쇄 폭발+/연쇄 방어+/임기응변+)과 UI 강조(5번)/선택 항목(6번)은 여전히
  미착수. 1~5번이 모두 끝나면 이 항목 전체를 "처리됨"으로 옮길 것(완료 기준,
  아래 원문 그대로 유지).
