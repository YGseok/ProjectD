...
- [부분 처리됨 - 2026-09-17] 2026-09-17 다음 할 거리로, 직업별 여러 스킬을 만들고,
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 E] 캐릭터별 시작 스킬 선택)**
  ...
  → **진행 상황(2026-09-17 (134)): 2번(시작 스킬 데이터)만 완료.**
  `code/systems/skill_pool.gd`에 위 6개 원형 그대로 `STARTING_SKILLS` 상수와
  `starting_skills_for_character(character_id)` 필터 헬퍼를 추가했다 —
  캐릭터별 슬롯 순서(견습=[확장,정예], 광전사=[맹공,수집가], 수호자=[철벽,강철
  방비], 폭발병=[맹공,확장], 방패병=[철벽,정예])가 위 원문과 정확히 일치함을
  `dice_test.gd`의 `_check_starting_skills`로 검증(`bash scripts/qa_shot.sh
  dice_test` 전체 PASS). 1번(해금 판정)/3번(선택 UI)/4번(적용 배선)은 아직
  미착수라, 지금은 이 데이터를 실제로 읽거나 쓰는 곳이 없어 게임 플레이에는
  영향이 전혀 없다(RunState/character_select.gd/combat_test.gd 전부 미변경).
  docs/STATUS.md 완료 기록(134)/다음 할 일 큐 19번 참고.
...
최근 12개만 유지해 매 이터레이션 읽기 비용을 줄임. 이번 이터레이션(133)에서
"키보드 단축키가 씬을 바꾸는 버튼에 배정됐을 때 크래시하는 버그" 항목을 그리로
옮겼다.)*
