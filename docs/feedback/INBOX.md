...
- 2026-09-15 (기획자 결정 — 아래 5건의 미착수 항목에 대한 구체적 설계) 밑에 그대로
  ...
  ~~**[미니 기획 A] 몬스터별 성격 디자인**~~ → **완료됨 (2026-09-16, 116/117/118) —
  아래 "처리됨" 참고.**
  ...
  **[미니 기획 C] 캐릭터 스킬 부여 이벤트** (신규 시스템 — 지금은 캐릭터 차별화가
  ...
  구조가 정리된 뒤) 시작하는 게 자연스러움) — **1~2번은 2026-09-16 (119)에서
  구현 완료, 3~5번(실제 효과 배선/고유 스킬/UI 다듬기) 남음. 아래 "남은 이슈"에
  그대로 두고 5번까지 전부 끝나면 "처리됨"으로 옮길 것 — docs/STATUS.md 완료
  기록(119) 참고:**
  ...
  기획 A]/[B] 관련 항목은 완료되어 아래 "처리됨"으로 옮겼다. [미니 기획 C]가 실제로
...
최근 12개만 유지해 매 이터레이션 읽기 비용을 줄임, 2026-09-16 정리.)*
