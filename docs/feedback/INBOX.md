...
- [부분 처리됨 - 2026-09-17] 2026-09-17 스킬을 강화시킬 수 있는 이벤트를
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 D] 스킬 강화 이벤트)**
  ...
  → **1~2번 완료(2026-09-17, docs/STATUS.md 완료 기록 (128)), 3~6번 남음.**
  ...
  두 함수를 추가했다 — 아직 이벤트 발생 구조(3번)/전투 배선(4번)/UI 강조(5번)/
  선택 항목(6번)은 미착수라 화면에는 아무 변화가 없다. 1~5번이 모두 끝나면
  이 항목 전체를 "처리됨"으로 옮길 것(완료 기준, 아래 원문 그대로 유지).
