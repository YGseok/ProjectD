...
- 2026-09-15 (기획자 결정 — 아래 5건의 미착수 항목에 대한 구체적 설계) 밑에 그대로
  ...
  **[미니 기획 A] 몬스터별 성격 디자인** (`code/scenes/combat_test.gd`의
  ...
  ~~**[미니 기획 B] 특수 이벤트 개편**~~ → **완료됨 (2026-09-16, 113/114/115) —
  아래 "처리됨" 참고.**
  ...
  **[미니 기획 C] 캐릭터 스킬 부여 이벤트** (신규 시스템 — 지금은 캐릭터 차별화가
  ...
  (아래는 위 결정이 나오기 전 원본 피드백 — 히스토리 보존용으로 그대로 둠. [미니
  기획 B] 관련 5건은 완료되어 아래 "처리됨"으로 옮겼다. [미니 기획 A]/[C]가 실제로
  구현되면 그 원본 항목도 함께 "처리됨"으로 옮길 것.)
