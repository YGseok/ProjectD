...
- [부분 처리됨 - 2026-09-17] 2026-09-17 스킬을 강화시킬 수 있는 이벤트를
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 D] 스킬 강화 이벤트)**
  ...
  공용 2종(심호흡+/여분+) 배선 완료(130), 나머지 고유 5종/UI 강조(5번)/선택
  항목(6번) 남음.** `code/systems/skill_pool.gd`에 `UPGRADE_SKILLS`(7종 강화판
  데이터) + `SkillPool.available_upgrade_choices(n, character_id)`/
  ...
  **(130)에서 4번의 첫 조각으로 공용 스킬 2종의 "+" 효과를 `combat_test.gd`의
  ...
  "`+` > base > 없음" 우선순위로 분기. 고유 스킬 5종(광기 심화+/수호 심화+/
