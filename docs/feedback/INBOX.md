...
- 2026-09-15 (기획자 결정 — 아래 5건의 미착수 항목에 대한 구체적 설계) 밑에 그대로
  ...
  **[미니 기획 B] 특수 이벤트 개편** (`code/scenes/event.gd` +
  ...
  ~~1. 이벤트 항목마다(또는 방 입장 시 공통으로) 1~2문장짜리 짧은 상황 문구를 추가한다
     ...
     패턴을 참고하면 재사용하기 쉬움.~~ → **완료됨 (2026-09-16 (114))**.
     `EventItemPool.ITEMS`에 `flavor` 필드를 추가하는 쪽을 택함(아이템 하나하나가
     곧 "이 상황에서 집어드는 물건"이라 문구를 분리할 이유가 없었음).
     `ItemCardStyle.build_card()`가 필드가 있으면 카드 제목 아래에 표시(다른
     화면의 DiceItemPool 카드는 영향 없음). 아래 "부분 처리됨" 참고.
  ...
  5. 위 1~4는 서로 의존하니 한 이터레이션에 전부 하지 말고 "4(이벤트 다이스 필드+
     ...
     진행할 것을 권장한다(각 조각이 독립적으로 QA 가능하도록). **-> 4번/1번 완료,
     다음은 2+3번.**
...
- [부분 처리됨 - 2026-09-16] 2026-09-15 [미니 기획 B] 특수 이벤트 개편 (원본:
  ...
  → 권장 순서(4 -> 1 -> 2+3) 중 **4번(이벤트 다이스 필드+시각화, (113)) /
  1번(방 진입 시 상황 문구, (114))까지 완료**. (113): `character_profiles.gd`/
  `run_state.gd`에 `event_die_sides`(전부 6) 필드, 신규 `code/scenes/
  event_die_visual.gd`(`EventDieVisual`)로 로마 숫자 육각 칩 시각화, 캐릭터
  선택 상세 패널에 샘플 표시. (114): `event_item_pool.gd`의 `ITEMS` 5종에
  1문장짜리 "flavor" 필드를 추가하고, `item_card_style.gd`의 공용 카드
  헬퍼가 그 필드가 있으면 카드 제목 아래에 표시하도록 함(다른 화면의
  DiceItemPool 카드는 영향 없음). `dice_test.gd`에 `_check_event_die_sides`/
  `_check_event_item_flavor` 신규 검증 추가, 회귀 스위트 전체 PASS. 남은 것:
  **2+3번**(선택지 "안전/위험" 2개 통일 + 이벤트 다이스를 실제로 굴려 DC와
  비교하는 난이도 체크) — 지금은 여전히 "2개 중 1개 무조건 성공 획득" 구조라
  flavor 문구는 왜 이 아이템을 얻었는지만 설명할 뿐 리스크/실패와 연결되지
  않는다. `docs/STATUS.md` 완료 기록(113, 114) 참고. [미니 기획 A](몬스터
  성격)/[미니 기획 C](캐릭터 스킬 이벤트)는 이번에도 손대지 않음 — 위 "남은
  이슈"에 그대로 남아있음.
