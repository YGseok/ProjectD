...
- [부분 처리됨 - 2026-09-17] 2026-09-17 다음 할 거리로, 직업별 여러 스킬을 만들고,
  ...
  **(기획자 결정 — 구체적 설계, [미니 기획 E] 캐릭터별 시작 스킬 선택)**
  ...
  → **진행 상황(2026-09-17 (137)): 1/2/3번 완료, 4번(적용 배선)은 6개 중
  4개(맹공/철벽/확장/정예) 완료, 2개 남음(5번은 선택).** (134)에서
  ...
  **구현 중 발견: 방패병의 "정예" 슬롯은 방패병 기본 다이스 합계(7개)가
  ...
  기록, 임계값/슬롯 배정을 임의로 바꾸지 않고 사람 결정을 기다림. **남은
  2개("수집가"/"강철 방비")는 아직 미배선이라, 그 2종을 시작 스킬로 고르면
  여전히 `chosen_starting_skill_id`/`skill_flags`에는 들어가지만 실제 전투
  보너스는 없다.** docs/STATUS.md 완료 기록(136, 137)/다음 할 일 큐 19번
  참고.
