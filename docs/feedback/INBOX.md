...
- 2026-09-17 다음 할 거리로, 직업별 여러 스킬을 만들고, 시작할 때 시작 스킬을
  하나 정하도록 하자. 최초에는 하나밖에 못고름. 나머지는 전부 lock 걸려있음.
  캐릭터별 업적 보상(해당 캐릭터로 라운드 클리어)하면 해금되어, 다음 회차에서
  다른 스킬을 선택할 수 있도록 선택지를 늘린다. 아직 변경된 스킬을 보진
  못했지만, 시작 스킬에 따라 게임 플레이 경험이나, 선택해야하는 주사위 밸류가
  달라지면 좋을 듯함. 공격 덱에 투자하면 이득이라던가, 방어 덱에 투자하면
  이득이라던가, 주사위가 많아지면 좋거나, 반대로 덜 늘리면 좋거나, 특정
  재질로 바꾸면 좋거나, 눈금을 모으면 좋거나 등등...
...
- [부분 처리됨 - 2026-09-17] 2026-09-17 스킬을 강화시킬 수 있는 이벤트를
  추가해줘. 강화되는 스킬은 [스킬명+] 라고 네이밍 붙여서, 기존 스킬의
  강화판임을 직관적으로 알 수 있게 한다. 좋아보여야 하니까 아이콘 테두리를
  노란 색으로 두껍게 강조하면 좋을 듯 함.

  **(기획자 결정 — 구체적 설계, [미니 기획 D] 스킬 강화 이벤트)**
  지금 캐릭터 스킬은 공용 2종("심호흡"/"여분")+고유 5종(캐릭터별 1개씩, 전부
  완료됨)이 전부 `RunState.skill_flags`에 문자열 id로만 쌓이고 강화 개념이 없다.
  아래 순서로 작은 단위로 나눠 구현할 것 — 사람 설계 결정을 더 기다리지 말 것.

  → **1~3번 완료(2026-09-17, docs/STATUS.md 완료 기록 (128)/(129)), 4번 중
  공용 2종(심호흡+/여분+, 130) + 고유 4종(광기 심화+/수호 심화+, 131 / 연쇄
  폭발+/연쇄 방어+, 132) 배선 완료, 나머지 고유 1종(임기응변+)/UI 강조(5번)/
  선택 항목(6번) 남음.** `code/systems/skill_pool.gd`에 `UPGRADE_SKILLS`(7종
  강화판 데이터) + `SkillPool.available_upgrade_choices(n, character_id)`/
  `grant_upgrade(base_id)` 두 함수를 추가했고((128)), 이어서 `code/scenes/
  event.gd`에 `SKILL_UPGRADE_EVENT_CHANCE`(=0.2, 잠정값)로 특수 이벤트 방이
  기존 스킬 이벤트(30%)보다 먼저 "스킬 강화" 이벤트로 분기하도록 구현했다
  ((129)) — 강화 후보가 하나도 없으면 항상 기존 스킬/아이템 이벤트로 폴백,
  카드 UI는 새로 만들지 않고 기존 스킬 획득 카드 UI를 그대로 재사용.
  **(130)에서 4번의 첫 조각으로 공용 스킬 2종의 "+" 효과를 `combat_test.gd`의
  `_do_exchange()`에 배선**했다 — "심호흡+"(매 방어턴 적용, base는 첫
  방어턴만)와 "여분+"(여분 다이스 2개 굴려 최댓값 채택, base는 1개)를 지시된
  "`+` > base > 없음" 우선순위로 분기. **(131)에서 광전사/수호자 페어(광기
  심화+/수호 심화+)를 이어서 배선** — 둘 다 base의 "1D20 두 번 굴려 최댓값"을
  "세 번 굴려 최댓값"으로 강화, 공용 로직은 신규 `combat_test._apply_bonus_
  reroll()` 순수 함수로 분리해 단위 테스트 가능하게 함. **(132)에서 폭발병/
  방패병 페어(연쇄 폭발+/연쇄 방어+)를 이어서 배선** — base(임계치만 2로
  낮춤, 보너스 턴 굴림 강화 없음)와 달리 "+"는 임계치는 2로 유지한 채 보너스
  턴 굴림을 광기/수호 심화 base와 같은 방식(1D20 한 번 더, 총 2번 굴려
  최댓값)으로 강화 — 같은 `_apply_bonus_reroll()` 헬퍼 재사용(신규 순수
  함수는 없음, 기존 (3c) 테스트가 extra_rolls=1 경우도 이미 커버). 남은 고유
  스킬 1종(임기응변+)과 UI 강조(5번)/선택 항목(6번)은 여전히 미착수. 1~5번이
  모두 끝나면 이 항목 전체를 "처리됨"으로 옮길 것(완료 기준, 아래 원문 그대로
  유지).

  1. **강화판 데이터 정의** (`code/systems/skill_pool.gd`): 기존 7종 스킬 각각에
     대응하는 "+" 버전을 `UPGRADE_SKILLS: Array[Dictionary]` 상수로 추가한다.
     각 항목은 `{id, name, description, upgrades: <base id>, character_id
     (고유 스킬 계열만)}` 형태(UNIQUE_SKILLS와 같은 필드 패턴 재사용). 이름은
     전부 "<기존 이름>+"로 통일(예: "심호흡+", "광기 심화+"). 효과는 아래처럼
     "기존 메커니즘을 한 단계 더 강하게"만 하고 새 메커니즘을 만들지 않는다:
     - `deep_breath_plus`("심호흡+"): 방어 다이스 +1 보정을 **첫 방어턴만이
       아니라 매 방어턴마다** 적용(상한은 기존처럼 각 다이스 면 개수).
     - `spare_die_plus`("여분+"): 여분 다이스를 **1개가 아니라 2개** 굴려, 그
       중 더 높은 값으로 가장 낮은 공격 다이스를 대체.
     - `frenzy_deepen_plus`("광기 심화+", 광전사 전용): 보너스 공격턴 굴림 횟수를
       **2번(기존)에서 3번**으로 늘려 그 중 최댓값 채택.
     - `guard_deepen_plus`("수호 심화+", 수호자 전용): 대칭 — 보너스 방어턴
       굴림 횟수 2번 -> 3번.
     - `chain_explosion_plus`("연쇄 폭발+", 폭발병 전용): 임계치는 2 그대로 두고,
       보너스 공격턴을 **1D20 한 번에서 두 번 굴려 최댓값 채택**으로 강화(기존
       chain_explosion엔 이 굴림 강화가 없었음 — frenzy_deepen이 쓰는 "두 번
       굴림"을 이번에 폭발병 쪽에도 가져오는 것).
     - `chain_guard_plus`("연쇄 방어+", 방패병 전용): 대칭 — 임계치 2 유지,
       보너스 방어턴 1D20 두 번 굴려 최댓값 채택.
     - `versatile_surge_plus`("임기응변+", 견습 모험가 전용): 공격/방어 두
       파이프라인의 임계치를 **3에서 2로** 낮춤(다른 4개 "+"가 "굴림 품질"을
       올리는 것과 달리 이건 "발동 빈도"를 올리는 축 — 임기응변은 원래 "얕지만
       넓게"였으니 이제 "얕지만 넓고, 더 자주"가 됨).
  2. **강화 후보 뽑기**: `SkillPool.available_upgrade_choices(n, character_id)`
     신규 함수 — `RunState.skill_flags`에 base id는 있는데 그 "+"id는 아직 없는
     스킬만 후보로 반환(공용 스킬은 character_id 무관하게 항상 후보, 고유 스킬은
     character_id가 일치할 때만). `SkillPool.grant_upgrade(base_id)` 헬퍼로
     `skill_flags`에 "+"id를 추가(기존 `grant()`와 같은 패턴, 중복 방지 포함).
  3. **이벤트 발생 구조** (`code/scenes/event.gd`): 특수 이벤트 방에서 일정
     확률(기존 `SKILL_EVENT_CHANCE`와 별개의 신규 상수, 예: 20% 잠정값)로
     "스킬 강화" 이벤트가 되도록 한다. **강화 가능한 스킬이 하나도 없으면
     (플레이어가 스킬을 아직 하나도 안 얻었거나 이미 다 강화한 경우) 반드시
     기존 아이템/스킬 획득 이벤트로 대체**(이미 있는 "스킬 후보 없으면 아이템
     이벤트로 대체"와 같은 폴백 패턴 재사용). 후보가 있으면 최대 2개 중 1개
     선택(기존 스킬 획득과 동일한 카드 UI 재사용).
  4. **전투 배선** (`code/scenes/combat_test.gd`): 위 7개 "+" 효과를 실제로
     적용 — 한 이터레이션에 7개를 다 하지 말고 나눠서 진행(예: 공용 2개 먼저,
     그 다음 고유 5개는 캐릭터 페어(광전사/수호자, 폭발병/방패병)나 하나씩
     나눠서). "+"가 있으면 그 효과를 쓰고 없으면 base 효과, base도 없으면
     기존처럼 아무 효과 없음 — 세 단계 우선순위(`+` > base > 없음)로 분기.
  5. **UI 강조** (`code/scenes/item_card_style.gd`): `ItemCardStyle.build_card()`
     에 `highlight: bool = false` 매개변수를 추가 — true면 카드 테두리를 평소
     등급별 색 대신 **두껍고 노란(Color.YELLOW 계열) 테두리**로 그린다("아이콘
     테두리"라는 원 표현은 지금 스킬 카드에 별도 아이콘이 없어 카드 자체의
     테두리로 구현 — 이 카드가 사실상 스킬을 나타내는 유일한 시각 요소이므로).
     강화 스킬 제안 카드에는 항상 `highlight=true`로 호출.
  6. **(선택, 여유 있으면)** 지금 획득한 스킬(`skill_flags`)을 상시 확인할 수
     있는 화면이 없다 — `DeckPanel`에 "보유 스킬" 목록(획득한 스킬 이름을
     나열, "+"가 붙은 강화판은 5번의 노란 테두리와 같은 색으로 강조)을 추가하면
     좋음. 급하지 않으니 1~5번을 먼저 끝내고 여유가 되면 진행.
  완료 기준: 각 하위 단계가 끝날 때마다 STATUS.md에 진행 상황을 남기고, 1~5번이
  모두 끝나면 이 항목 전체를 "처리됨"으로 옮길 것.
