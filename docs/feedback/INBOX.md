...
- [부분 처리됨 - 2026-09-15] 2026-09-14 키보드로도 조작이 되도록 키매핑 및 단축키를
  ...
  바로 아래 별도 항목("씬 전환 버튼 크래시 버그") 참고. **2026-09-15 (106)에
  전투 화면(`combat_test.gd`)도 추가**했다(6/7화면) — 전투는 다른 화면과 달리
  버튼 구성이 시간에 따라 계속 바뀌므로(대기 중 "덱 보기"뿐 -> 승패 직후
  "커스터마이징" 추가 -> 승리 시 보상 카드(가변 개수)+커스터마이징+건너뛰기 ->
  "다음") 신규 `_rebuild_shortcuts()` 함수로 버튼 구성이 바뀌는 지점마다 다시
  배정하도록 만들었다. (105)에서 발견한 크래시 패턴(씬을 바꾸는 버튼을
  `try_press()`로 누른 뒤에 `get_viewport()`를 부르면 크래시)을 이미 알고
  있었으므로 `combat_test.gd`는 처음부터 안전한 순서로 작성했고, 전투 화면에서
  유일하게 씬을 바꾸는 "다음" 버튼을 실제 숫자 키 입력으로 눌러
  `qa_out/combat_test_kb_key_next.png`로 크래시 없이 dungeon_map으로 정상
  전환됨을 확인했다. `qa_out/combat_test_kbshortcuts_battle.png`(전투 중
  "[1] 덱 보기"만)/`qa_out/combat_test_kbshortcuts_reward2.png`(승리 보상
  화면, 카드 버튼+커스터마이징+건너뛰기가 [1]~[6], 덱 보기가 [7])/
  `qa_out/combat_test_kb_key_skip.png`(건너뛰기를 숫자 키로 눌러 "다음"이
  올바른 번호로 재배정됨)로 화면 확인, `dice_test.gd` 회귀 스위트 전체 PASS.
  **아직 적용 안 된 화면**: 업적 패널(`achievement_panel.gd`, "닫기" 버튼
  하나뿐이라 단축키 실익이 적음) — 같은 `KeyboardShortcuts` 유틸을 그대로
  재사용할 수 있는 구조라 다음 이터레이션이 이어서 적용하면 됨(`docs/STATUS.md`
  다음 할 일 큐 16번 "키보드 조작/단축키" 참고, 6/7화면). "전체 조작을
  키보드로도"라는 원 요청 전체를 아직 다 커버한 것은 아니라 완전히 처리된
  것은 아님.
