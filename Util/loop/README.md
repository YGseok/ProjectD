# Util/loop/ — 헤드리스 반복 개발 루프

## 시작

```bash
chmod +x Util/loop/loop.sh scripts/qa_shot.sh
./Util/loop/loop.sh
```
...
다시 시작하려면 STOP 파일을 지우고 `./Util/loop/loop.sh` 를 다시 실행하면 된다:
...
각 이터레이션의 전체 출력은 `Util/loop/logs/iter_<N>_<timestamp>.log` 에 저장된다.
